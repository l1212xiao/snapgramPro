...
@Service
public class MyOdataEdmProvider extends CsdlAbstractEdmProvider {

    // Service Namespace
    public static final String NAMESPACE = Constants.NAMESPACE;

    // EDM Container
    public static final String CONTAINER_NAME = Constants.CONTAINER_NAME;
...
//        if (entityTypeName.equals(ET_PRODUCT_FQN)) {
        ...
        TableMapping table = Utils.getTable(entityTypeName.getName());
        String tbName = table.getTbName();
        Integer tableId = table.getId();

        List<FieldMapping> fields = Utils.getFieldMapping(tableId);
        for (FieldMapping field : fields) {
            new CsdlProperty().setName(field.getFieldName())
                    .setType(Objects.requireNonNull(Utils.getType(field.getFieldType())));
        }
...
//            entityType.setNavigationProperties(navPropList);
    ...
    public CsdlEntitySet getEntitySet(FullQualifiedName entityContainer, String entitySetName) {
        ...
        if (entityContainer.equals(CONTAINER)) {
            ...
            TableMapping table = Utils.getTable(entitySetName);
            String tbName = table.getTbName();
            Integer tableId = table.getId();


            entitySet = new CsdlEntitySet();
            entitySet.setName(tbName);
            entitySet.setType(new FullQualifiedName(NAMESPACE, tbName));
...
//    }
    ...
    public List<CsdlSchema> getSchemas() {
        ...
        List<TableMapping> tableMapping = Utils.getTableMapping();
        for (TableMapping table : tableMapping) {
            entityTypes.add(getEntityType(new FullQualifiedName(NAMESPACE, table.getTbName())));
        }
...
//        schema.setFunctions(functions);
    ...
    public CsdlEntityContainer getEntityContainer() {
        ...
        List<TableMapping> tableMapping = Utils.getTableMapping();
        for (TableMapping table : tableMapping) {
            entitySets.add(getEntitySet(CONTAINER, table.getTbName()));
        }
